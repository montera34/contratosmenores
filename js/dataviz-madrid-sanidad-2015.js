...
d3.tsv("data/viplist_mad2015.tsv", function(error, data) {//reads the viplist.tsv file
...
//Legend de cosas
d3.tsv("data/thinglist_mad2015.tsv", function(error, data) {//reads the thinglist.tsv file
...
d3.tsv("data/centroslist_mad2015.tsv", function(error, data) {//reads the thinglist.tsv file
	legendcentros.selectAll('div')
		...
		.text(function(d) { return d.centro; }) //Elige el idioma de la leyenda
		.on('click',function(d) { //when click on name
			var centro = replacement(d.centro.toLowerCase().replace(/\.+/g, ''));
...
d3.tsv("data/data_mad2015.tsv", type, function(error, data) {//reads the data.tsv file
...
*/
	...
	barstimescale.selectAll(".bar")
		...
		.attr("class",
			function(d) {
				return replacement(d.quien) + " bar " + replacement(d.centro.toLowerCase().replace(/\.+/g, '')) + "  " + d.actividad.replace(/\,+/g, ' ').toLowerCase() + " " + (d.importe < 0 ? " negativo" : " positivo"); 
	...
	specialdates.append("text")
		...
		.attr("x", function(d) { return xScale(parseDate('05/25/2015')) + 6; })
		.attr("y", height+55)
	...
	specialdates.append('line')
		...
		.attr('y1', height+44)
		.attr('y2', height+60)
		.attr('x1', function(d) { return xScale(parseDate('05/24/2015')) + 1; })
		.attr('x2', function(d) { return xScale(parseDate('05/24/2015')) + 1; })
